...
- [x] El front exporta datos pendientes desde SQLite local y envía payload válido a `/api/sqlite`.
- [x] El backend importa personas/controles/relaciones por UUID sin duplicar registros existentes.
- [x] Reintentar el mismo payload no genera duplicados (idempotencia validada).
- [x] Las bajas lógicas (`sql_deleted`) quedan reflejadas en MySQL.
- [x] Las relaciones críticas no dependen de IDs locales para consolidación final.
- [x] Se generan métricas por import (totales por tabla + rechazados por motivo).
- [x] Se dispone evidencia de pruebas E2E (pasos + resultados esperados/obtenidos).
...
## Cierre
- 2026-04-19: tarea cerrada luego de hardening front/back UUID-first, ajustes de persistencia local y validación funcional E2E.
