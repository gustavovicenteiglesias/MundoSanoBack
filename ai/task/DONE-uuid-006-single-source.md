...
## Cierre
- 2026-05-29: se cierra administrativamente durante reconstruccion de estado. La sincronizacion UUID/offline se considera saldada; queda documentado que los datos duros de tablas base pueden requerir verificacion puntual si se retoma el bootstrap dinamico.
...
- [x] Refactorizar `ExportControler.java` y Servicios de exportación para serializar el esquema de las tablas junto con los arrays de registros en el Formato de `JsonSQLite`.
...
- [x] Eliminar `mundoSano/src/data/exportarIII..last.js` o inutilizar su contenido.
- [x] Ajustar `CargarBase.ts` para que pase el cuerpo del endpoint directo a `sqlite.importFromJson`.
