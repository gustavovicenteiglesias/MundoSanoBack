...
async createBatch(meta: SyncMetaPayload, totalItems: number):
Promise<void> {
await withSyncLogsDb(async (db) => {
await db.execute(
`INSERT INTO sync_batch_log_local
 (sync_batch_id, usuario, dispositivo, version_app, fecha_inicio,
estado, total_items, ok_count, rejected_count, conflict_count, mensaje)
 VALUES (
...
`SELECT sync_batch_id, usuario, dispositivo, version_app,
fecha_inicio, fecha_fin, estado, total_items, ok_count, rejected_count,
conflict_count, mensaje
...
async getBySyncBatchId(syncBatchId: string): Promise<SyncBatchLogLocal |
null> {
return withSyncLogsDb(async (db) => {
const res: any = await db.query(
`SELECT sync_batch_id, usuario, dispositivo, version_app,
