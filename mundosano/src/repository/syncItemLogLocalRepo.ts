      ...
      for (const item of items) {
        await db.execute(
          `INSERT INTO sync_item_log_local
            (sync_batch_id, tabla, uuid, id_persona, id_control, id_referencia, estado, motivo, payload_json, created_at)
           VALUES (
  ...
  async getBySyncBatchId(syncBatchId: string): Promise<SyncItemLogLocal[]> {
    return withDb(async (db) => {
      const res: any = await db.query(
        `SELECT id_item, sync_batch_id, tabla, uuid, id_persona, id_control, id_referencia, estado, motivo, payload_json, created_at
